...
class CalendarCore extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PageView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final visibleDays =
            _daysInRange(calendarFormat, visibleRange.start, visibleRange.end);
  ...
  DateTime _getFocusedDay(
    ...
    switch (format) {
      case CalendarFormat.year:
        day = DateTime.utc(prevFocusedDay.year + pageDif);
        break;
      case CalendarFormat.month:
  ...
  DateTime _getBaseDay(CalendarFormat format, int pageIndex) {
    ...
    switch (format) {
      case CalendarFormat.year:
        day = DateTime.utc(firstDay.year + pageIndex);
        break;
      case CalendarFormat.month:
  ...
  DateTimeRange _daysInYear(DateTime focusedDay) {
    final firstToDisplay = DateTime.utc(focusedDay.year, 1, 1);
    final lastToDisplay = DateTime.utc(focusedDay.year, 12, 1);
    return DateTimeRange(start: firstToDisplay, end: lastToDisplay);
  }
    ...
    switch (format) {
      case CalendarFormat.year:
        ...
        return List.generate(
          monthCount,
          (index) => DateTime.utc(first.year, first.month + index, first.day),
        );
      default:
